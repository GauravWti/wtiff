...
import FullAndFinalRaw from "./FullAndFinalRaw";
...
const FunAndFinalInvoice=()=>{
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://wticarrental.ae:3000/app/v1/invoice/getAllFinalInvoice?page=${currentPage}`, {
          method: 'GET',
          headers: {
  ...
  return (
    ...
    <>
       {
        data?<>
        <h1 className="text-4xl font-bold text-blue-700 text-center">Download Final Invoice </h1>
       <div className="container mx-auto p-4 overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-300 shadow-lg">
          <thead>
            <tr className="text-sm bg-black text-white">
              <th className="sm:py-2 sm:px-2 border-b">invoiceID</th>
          ...
          <tbody>
            {data?.map((item, key) => (
              <FullAndFinalRaw data={item} key={key} />
            ))}
...
export default FunAndFinalInvoice;